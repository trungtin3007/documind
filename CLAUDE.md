...
- **Source URLs are recorded here now** (they were only in shell history before): USGS `https://pubs.usgs.gov/periodicals/mcs2025/mcs2025.pdf` · Fed `https://www.federalreserve.gov/monetarypolicy/files/20250620_mprfullreport.pdf` · Census `https://www2.census.gov/library/publications/2024/demo/p60-282.pdf` — all three re-verified HTTP 200. **BLS has no recorded URL** (the developer downloaded it manually), so its card is listed deliberately **without a link** rather than guessing one. Add the URL to the `bls-ce-2023` card in `static/index.html` when known.
