...
- **Next: Render deploy (NOT started).** Open items: the app needs `GEMINI_API_KEY` + `VOYAGE_API_KEY` as env vars, `DOCUMIND_DATA=demo`, `DOCUMIND_ANSWER_MODEL=gemini-3.6-flash`; sentence-transformers downloads ~90MB on first boot (cold start); free-tier Gemini quota is per-day and shared with the judge model.
